...
def softmax_loss_naive(W, X, y, reg):
  ...
  # regularization!                                                           #
  #############################################################################
  numderOfElements = X.shape[0]
  numberOfClasses = W.shape[1]
  for i in range(numderOfElements):
    scores = np.array([np.exp(elem) for elem in X[i].dot(W)])
    normVal = np.sum(scores)
    correctClassScore = scores[y[i]]
    margin = -np.log(correctClassScore / normVal)
    loss += margin
    mask = np.array([elem / normVal for elem in scores])
    mask[y[i]] -= 1
